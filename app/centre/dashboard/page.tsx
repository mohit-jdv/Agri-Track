...
export default function CentreDashboard() {
  ...
  async function getAuthenticatedCentre() {
    ...
    const storedCentreId = localStorage.getItem("agritrack-centre-id");
    ...
    const { data, error } = await supabase
      .from("centres")
      .select("id, name, avg_processing_minutes, is_active")
      .eq("id", storedCentreId)
      .eq("auth_user_id", session.user.id)
      .maybeSingle();

    if (error || !data) {
      console.error("Centre authentication/link error:", error);
      await supabase.auth.signOut();
    ...
    const info: CentreInfo = {
      id: data.id,
      name: data.name,
      avg_processing_minutes: Number(data.avg_processing_minutes) || 15,
  ...
  async function loadProcurementStatus(token: string) {
    const centreInfo = centre ?? await getAuthenticatedCentre();
    ...
    if (!data) {
      console.error("No procurement record found for token:", token);
      setProcurementStatus(null);
      return;
    }
        ...
        <div className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            ...
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-[#5F8F45]">
              {centre?.name.replace(" Procurement Centre", "")} · Today
            </p>
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-[#172019]/50">
                ...
                {centre?.avg_processing_minutes ?? 15} minutes per farmer.
              </p>
            </div>
          </div>
